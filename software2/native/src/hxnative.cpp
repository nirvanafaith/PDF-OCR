    ...
    m.def("batch_crop_qimage",
          [](py::buffer page_rgba, int w, int h,
             py::list bboxes, int padding) -> py::list {
              ...
              // 校验缓冲区大小,防止越界访问导致原生崩溃
              const std::size_t expected = static_cast<std::size_t>(w) * static_cast<std::size_t>(h) * 4;
              if (info.size < static_cast<py::ssize_t>(expected)) {
                  throw std::runtime_error(
                      "batch_crop_qimage: buffer too small (got " +
                      std::to_string(info.size) + " bytes, need " +
                      std::to_string(expected) + " bytes for " +
                      std::to_string(w) + "x" + std::to_string(h) + " RGBA)");
              }
              const std::uint8_t *ptr = static_cast<const std::uint8_t *>(info.ptr);
          ...
          [](py::buffer samples, int width, int height,
             const std::string &mode, std::ptrdiff_t stride) -> py::bytes {
              ...
              if (info.ndim != 1)
                  throw std::runtime_error("samples must be a 1-D buffer");
              // 校验缓冲区大小,防止越界访问导致原生崩溃
              int channels = (mode == "RGBA") ? 4 : 3;
              const std::size_t expected = static_cast<std::size_t>(width) * static_cast<std::size_t>(height) * channels;
              if (info.size < static_cast<py::ssize_t>(expected)) {
                  throw std::runtime_error(
                      "pil_to_qimage_buffer: buffer too small (got " +
                      std::to_string(info.size) + " bytes, need " +
                      std::to_string(expected) + " bytes for " +
                      std::to_string(width) + "x" + std::to_string(height) + " " + mode + ")");
              }
              const std::uint8_t *ptr = static_cast<const std::uint8_t *>(info.ptr);
              std::ptrdiff_t given_stride = stride;
              if (given_stride <= 0) {
